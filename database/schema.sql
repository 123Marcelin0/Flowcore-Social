...
-- Create view for user dashboard summary (AFTER all tables and permissions are set)
...
SELECT 
    ...
    up.created_at,
    (SELECT COUNT(*)::INTEGER FROM posts p WHERE p.user_id = up.id) as total_posts,
    (SELECT COUNT(*)::INTEGER FROM posts p WHERE p.user_id = up.id AND p.status = 'published') as published_posts,
    (SELECT COUNT(*)::INTEGER FROM posts p WHERE p.user_id = up.id AND p.status = 'draft') as draft_posts,
    (SELECT COUNT(*)::INTEGER FROM posts p WHERE p.user_id = up.id AND p.status = 'scheduled') as scheduled_posts,
    (SELECT COUNT(*)::INTEGER FROM social_accounts sa WHERE sa.user_id = up.id) as connected_accounts,
    (SELECT COUNT(*)::INTEGER FROM content_ideas ci WHERE ci.user_id = up.id) as total_content_ideas,
    (SELECT COUNT(*)::INTEGER FROM interactions i WHERE i.user_id = up.id) as total_interactions,
    (SELECT COUNT(*)::INTEGER FROM interactions i WHERE i.user_id = up.id AND i.status = 'pending') as pending_interactions,
    (SELECT COALESCE(SUM(p.likes), 0)::INTEGER FROM posts p WHERE p.user_id = up.id) as total_likes,
    (SELECT COALESCE(SUM(p.comments), 0)::INTEGER FROM posts p WHERE p.user_id = up.id) as total_comments,
    (SELECT COALESCE(SUM(p.shares), 0)::INTEGER FROM posts p WHERE p.user_id = up.id) as total_shares
