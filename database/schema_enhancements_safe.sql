-- Social Media Dashboard Schema Enhancements - SAFE VERSION
-- This script safely handles existing tables and prevents conflicts
...
-- ==================================================
-- SAFE TABLE CREATION WITH CONFLICT HANDLING
-- ==================================================

-- Drop existing enhancement tables if they exist (clean slate approach)
-- Comment out any DROP statements for tables you want to keep
...
DROP TABLE IF EXISTS media_post_associations CASCADE;
...
-- Add organization_id columns to existing tables (safely)
DO $$ 
BEGIN
    -- Add organization_id to posts if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                  WHERE table_name = 'posts' AND column_name = 'organization_id') THEN
        ALTER TABLE posts ADD COLUMN organization_id UUID REFERENCES organizations(id);
    END IF;

    -- Add organization_id to media_files if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                  WHERE table_name = 'media_files' AND column_name = 'organization_id') THEN
        ALTER TABLE media_files ADD COLUMN organization_id UUID REFERENCES organizations(id);
    END IF;

    -- Add organization_id to content_ideas if the table exists and column doesn't
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'content_ideas') THEN
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                      WHERE table_name = 'content_ideas' AND column_name = 'organization_id') THEN
            ALTER TABLE content_ideas ADD COLUMN organization_id UUID REFERENCES organizations(id);
        END IF;
    END IF;

    -- Add organization_id to calendar_events if the table exists and column doesn't
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'calendar_events') THEN
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                      WHERE table_name = 'calendar_events' AND column_name = 'organization_id') THEN
            ALTER TABLE calendar_events ADD COLUMN organization_id UUID REFERENCES organizations(id);
        END IF;
    END IF;

    -- Add organization_id to social_accounts if the table exists and column doesn't
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'social_accounts') THEN
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                      WHERE table_name = 'social_accounts' AND column_name = 'organization_id') THEN
...
-- Vector index (only if pgvector extension is properly installed)
DO $$ 
BEGIN
    CREATE INDEX idx_hashtags_embedding ON hashtags USING ivfflat (embedding vector_cosine_ops);
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Could not create vector index - pgvector extension may not be properly installed';
END $$;
...
-- Drop existing trigger if it exists
DROP TRIGGER IF EXISTS trigger_update_publishing_retry ON publishing_queue;
...
DROP POLICY IF EXISTS "users_can_manage_own_post_hashtags" ON post_hashtags;
...
DROP POLICY IF EXISTS "users_can_manage_own_media" ON media_files;
DROP POLICY IF EXISTS "users_can_view_org_media" ON media_files;
DROP POLICY IF EXISTS "anyone_can_read_hashtags" ON hashtags;
DROP POLICY IF EXISTS "authenticated_users_can_create_hashtags" ON hashtags;
DROP POLICY IF EXISTS "users_can_manage_own_queue" ON publishing_queue;
DROP POLICY IF EXISTS "members_can_read_org" ON organizations;
DROP POLICY IF EXISTS "owners_can_manage_org" ON organizations;
DROP POLICY IF EXISTS "users_can_manage_own_embeddings" ON user_embeddings;
DROP POLICY IF EXISTS "users_can_manage_own_ai_suggestions" ON ai_suggestions;
DROP POLICY IF EXISTS "users_can_manage_own_content_analysis" ON content_analysis;
...
-- Function to find similar hashtags using embeddings (only if vector extension works)
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Vector similarity search failed - pgvector extension may not be properly installed';
        RETURN;
END;
$$ LANGUAGE plpgsql;
