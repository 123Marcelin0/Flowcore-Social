          ...
          <div className="absolute inset-0 rounded-[14px] border border-white/[0.04] p-4" style={{ background: 'radial-gradient(circle at 30% 30%, #3a3a3a 0%, #2a2a2a 25%, #1a1a1a 70%, #0a0a0a 100%)', boxShadow: 'inset 3px 3px 6px rgba(255,255,255,0.06), inset -3px -3px 6px rgba(0,0,0,0.4), 0 2px 8px rgba(0,0,0,0.3)' }}>
            <div className="grid grid-rows-[40px_1px_96px_1px_48px] h-full gap-3 text-white/70 pt-1">
              <div className="grid grid-cols-2 gap-3 h-full place-items-center">
                ...
                <button className={`w-9 h-9 rounded-[10px] flex items-center justify-center transition-colors ${showTextElements ? 'bg-white/15 text-white/90' : 'bg-white/8 text-white/50'}`} onClick={onToggleTextElements} title={showTextElements ? 'Hide text elements' : 'Show text elements'}>
              ...
              <div className="grid grid-cols-2 gap-3 h-full place-items-center">
                ...
                <button className={`w-9 h-9 rounded-[10px] flex items-center justify-center transition-colors ${showVideoClips ? 'bg-white/15 text-white/90' : 'bg-white/8 text-white/50'}`} onClick={onToggleVideoClips} title={showVideoClips ? 'Hide video clips' : 'Show video clips'}>
              ...
              <div className="grid grid-cols-2 gap-3 h-full place-items-center">
                <button className="w-9 h-9 rounded-[10px] bg-white/8 flex items-center justify-center text-white/80"><Lock className="w-5 h-5" style={{ strokeWidth: 1.6 }} /></button>
                <button className={`w-9 h-9 rounded-[10px] flex items-center justify-center transition-colors ${showAudioElements ? 'bg-white/15 text-white/90' : 'bg-white/8 text-white/50'}`} onClick={onToggleAudioElements} title={showAudioElements ? 'Hide audio elements' : 'Show audio elements'}>
              ...
              <div className="flex items-center gap-4">
                <div className="flex-1 relative" style={{ height: '60px' }}>
                  {textOverlays.map((t, i) => (
                    <div key={t.id} className="absolute bottom-0 h-[56px] rounded-[18px] text-white/90 overflow-hidden flex items-center" style={{ left: t.start * pixelsPerSecond, width: Math.max(120, t.duration * pixelsPerSecond), background: 'linear-gradient(135deg, #5a5a5a 0%, #4a4a4a 25%, #3a3a3a 50%, #2a2a2a 75%, #1a1a1a 100%)', boxShadow: 'inset 2px 2px 4px rgba(255,255,255,0.08), inset -2px -2px 4px rgba(0,0,0,0.5), 0 3px 10px rgba(0,0,0,0.4)' }} title={t.text}>
              ...
              <div className="flex items-center gap-4">
                <div className="flex-1 relative" style={{ height: '90px' }}>
                  {demoClips.map((clip, i) => (
                    <div key={clip.id} className="absolute bottom-0 h-[84px] rounded-[16px] bg-white border border-white/20 overflow-hidden shadow-[0_8px_22px_rgba(0,0,0,0.25)]" style={{ left: clip.start * pixelsPerSecond, width: clip.duration * pixelsPerSecond }}>
              ...
              <div className="flex items-center gap-4">
                <div className="flex-1 relative" style={{ height: '60px' }}>
                  {(() => {
                    ...
                    <div key={`audio-${i}`} className="absolute bottom-0 h-[52px] rounded-[18px] text-white/90 overflow-hidden flex items-center" style={{ left: segment.start * pixelsPerSecond, width: segment.duration * pixelsPerSecond, background: 'radial-gradient(circle at 30% 30%, #4a4a4a 0%, #3a3a3a 25%, #2a2a2a 70%, #1a1a1a 100%)', boxShadow: 'inset 3px 3px 6px rgba(255,255,255,0.06), inset -3px -3px 6px rgba(0,0,0,0.4), 0 2px 8px rgba(0,0,0,0.3)' }}>
                      <div className="relative w-10 h-10 rounded-[10px] border border-white/25 flex items-center justify-center overflow-hidden ml-3" style={{ background: 'rgba(255, 255, 255, 0.12)', backdropFilter: 'blur(10px)', boxShadow: '0 4px 12px rgba(0, 0, 0, 0.4), inset 0 1px 2px rgba(255, 255, 255, 0.2)' }}>
                        <span aria-hidden className="pointer-events-none absolute inset-x-0 bottom-0 h-2" style={{ background: 'linear-gradient(0deg, rgba(255,255,255,0.3) 0%, rgba(255,255,255,0) 85%)' }} />
                      ...
                      <div className="relative h-8 mx-2 flex-1 overflow-hidden">
                        <svg viewBox={`0 0 ${segment.duration * pixelsPerSecond} 32`} className="absolute inset-0 w-full h-full" preserveAspectRatio="none">
                          ...
                          {(() => {
                            ...
                            return (
                              <polyline points={points.join(' ')} fill="none" stroke={`url(#audioWaveGrad${i})`} strokeWidth="2" opacity="0.9" />
                            )
                          })()}
                        </svg>
                      </div>
