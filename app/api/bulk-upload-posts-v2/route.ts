...
function transformPostData(rawPost: any) {
  const {
    caption = '',
    content = '',
    description = '',
  ...
  return {
    ...
    metadata: {
      ...
      original_data: rawPost
    }
  };
}

// POST /api/bulk-upload-posts-v2 - Upload posts using SQL function
export async function POST(request: NextRequest) {
  try {
    ...
    // Use the SQL function to insert posts
    const { data, error } = await supabase.rpc('bulk_insert_user_posts', {
      posts_data: transformedPosts
    });

    if (error) {
      console.error('Error calling bulk insert function:', error);
      
      // If function doesn't exist, provide instructions
      if (error.message.includes('function bulk_insert_user_posts')) {
        return NextResponse.json({
          success: false,
          error: 'Bulk insert function not found. Please run the SQL setup first.',
          instructions: 'Run the SQL commands from database/fix-bulk-upload.sql in your Supabase SQL Editor'
        }, { status: 400 });
      }
    ...
    const insertedPosts = data || [];
    const totalInserted = Array.isArray(insertedPosts) ? insertedPosts.length : 0;

    return NextResponse.json({
      success: true,
      message: `Successfully uploaded ${totalInserted} posts using SQL function`,
      totalInserted,
      posts: insertedPosts
    }, { status: 201 });

  } catch (error) {
    console.error('POST /api/bulk-upload-posts-v2 error:', error);
    return NextResponse.json(
